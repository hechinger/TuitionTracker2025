...
const sequenceOf = (s: AdminField, defaultValue: string | AdminState) => ({
  type: "array" as const,
  key: "",
  label: "",
  defaultValue,
...
export const content = section({
  ...
  RecommendedSchools: section({
    sections: sequenceOf(section({
      title: txt("Big State Schools"),
      schoolIds: sequenceOf(schoolId(), ""),
    }), { title: "", schoolIds: [] }),
  }),
  Newsletter: section({
  ...
  Recirculation: section({
    title: txt("Read more"),
    articles: sequenceOf(section({
      url: str("https://hechingerreport.org/"),
      headline: str(""),
      image: str(""),
      imageAlt: str(""),
    }), { url: "", headline: "", image: "", imageAlt: "" }),
  }),
  About: section({
    title: txt("About"),
